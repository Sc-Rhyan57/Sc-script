...
function Script.Functions.AutoWardrobe(child, index)
    ...
    if Toggles.AutoWardrobeNotif.Value then
        OrionLib:MakeNotification({
            Name = NotifPrefix,
            Content = "Procurando um esconderijo...",
    ...
    local isSafeCheck = function(addMoreDist)
        ...
        for _, entity in pairs(Script.Temp.AutoWardrobeEntities) do
            ...
            local inView = Script.Functions.IsInViewOfPlayer(entity.PrimaryPart, EntityTable.AutoWardrobe.Distance[entity.Name] + (addMoreDist and 15 or 0), exclusion)
            local isClose = Script.Functions.DistanceFromCharacter(entity:GetPivot().Position) < 45 + (addMoreDist and 15 or 0)
            isSafe = entityDeleted or (inView == false and isClose == false)
            if not isSafe then break end
        end
        return isSafe
    end

    local waitForSafeExit = function()
        repeat task.wait(.15)
            if isSafeCheck(true) then
                if Toggles.AutoWardrobeNotif.Value then
                    OrionLib:MakeNotification({
                        Name = NotifPrefix,
                        Content = "Saindo do esconderijo, entidade distante.",
                        Image = "rbxassetid://4483345998",
                        Time = 5
                    })
                end
        ...
        until not child:IsDescendantOf(workspace) or not alive
    end
...
autoIn:AddToggle({
    ...
    Default = false,
    Callback = function(Value)
        Toggles.AutoWardrobe.Value = Value
    end    
})
